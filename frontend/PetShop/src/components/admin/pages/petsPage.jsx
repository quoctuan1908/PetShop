...
export const PetsPage = () => {
  ...
  const fetchPets = async () => {
    const getPets = await petService.findAll("*");
    setPets(getPets);
    setListForSearch(getPets);
  };
  ...
  const columns = [
    ...
    { id: "species_id", label: "Species", minWidth: 80 },
    {
      id: "pet_name",
      label: "Pet name",
      minWidth: 80,
    ...
    {
      id: "selling_prices",
      label: "Selling prices (Import prices)",
      minWidth: 50,
    },
    {
      id: "quantity",
    ...
    <Paper sx={{ width: "90%" }}>
      <div className="grid grid-cols-4">
        <div className="text-left col-span-1 p-3">
          <AddPetModal handleRefresh={fetchPets} />
        </div>
      ...
      <TableContainer sx={{ maxHeight: 600 }}>
        <Table sx={{ minWidth: 500 }} aria-label="table pagination ">
          ...
          <TableBody>
            {(rowsPerPage > 0
              ? pets.slice(page * rowsPerPage, page * rowsPerPage + rowsPerPage)
              : pets
            ).map((pet) => {
              return (
                <TableRow
                ...
                >
                  ...
                  <TableCell>
                    <span
                      className="inline-block w-20 overflow-hidden text-ellipsis whitespace-nowrap"
                      title={pet.species_id}
                    >
                      {pet.species_id}
                    </span>
                  </TableCell>
                  <TableCell>{pet.pet_name}</TableCell>
                  <TableCell>{pet.sex ? "Male" : "Female"}</TableCell>
                  <TableCell>{pet.weight}</TableCell>
                  <TableCell>{pet.age}</TableCell>
                  <TableCell>
                    {pet.selling_prices} $ ({pet.import_prices} $)
                  </TableCell>
                  <TableCell>{pet.quantity}</TableCell>
                  <TableCell>{pet.description}</TableCell>
                  <TableCell>
                    <EditPetForm pet={pet} handleRefresh={fetchPets} />
                    <Button
                      variant="outlined"
                      onClick={(event) => handleDeleteItem(event, pet.id)}
                    >
