...
export const OrdersHistoryPage = () => {
  ...
  const handleDeleteItem = async (event, id) => {
    ...
    const result = await orderService.delete(id, staff_id);
    if (result) {
  ...
  const handleFinishDelivering = async (event, id) => {
    event.preventDefault();
    const staff_id = JSON.parse(localStorage.getItem("account")).id;
    const result = await orderService.finishDelivering(id, staff_id);
    if (result) {
      showAlert("Delivering successfully", "success");
    } else showAlert("Delete failed", "error");
    console.log(result);
    await fetchOrders();
  };
  ...
  const fetchOrders = async () => {
    const getOrders = await orderService.findHistory(null);
                    ...
                    <TableCell>
                      {order.status === "DELIVERING" ? (
                        <Button
                          variant="outlined"
                          onClick={(event) =>
                            handleFinishDelivering(event, order.id)
                          }
                        >
